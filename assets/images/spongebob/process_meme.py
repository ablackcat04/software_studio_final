...
# 使用的 Gemini 模型名稱
MODEL_NAME = "gemini-2.5-falsh-preview-05-20" # 或其他你指定的 preview 版本，例如 "gemini-1.5-flash-preview-0514"
...
def get_image_files_and_ids(folder_path):
    """獲取資料夾中所有 .png 圖片的路徑和對應的 ID，並按 ID 排序"""
        ...
        for filename in os.listdir(folder_path):
            if filename.lower().endswith(".png"):
                ...
                if match:
                    ...
                    log_error(f"檔案 {filename} 不符合 '數字.png' 格式，已跳過。")
    except Exception as e:
...
def main():
    ...
    if not image_files:
        print(f"在指定的資料夾 ({IMAGE_FOLDER_PATH}) 中沒有找到符合 '數字.png' 格式的圖片檔案。")
        return
    ...
    while True:
        ...
        for img_info in image_files:
            ...
            # 只處理那些是 '數字.png' 格式且 ID 不在已處理列表中的圖片
            if image_id_str not in processed_ids and f"{img_info['id']}.png" in os.path.basename(img_info['path']).lower():
